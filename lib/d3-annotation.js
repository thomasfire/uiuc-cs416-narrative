...
var addHandles = function addHandles(_ref5) {
  ...
  h.enter().append("circle").attr("class", "handle").attr("fill", "grey").attr("fill-opacity", 0.7).attr("cursor", "move").attr("stroke-dasharray", 5).attr("stroke", "grey").call(d3Drag.drag().container(d3Selection.select("g.annotations").node()).on("start", function (d) {
  ...
  createClass(d3NoteText, [{
    ...
    value: function drawText() {
      if (this.note) {
        ...
        this.a.select("rect.annotation-note-bg").attr("width", bbox.width + bgPaddingFinal.left + bgPaddingFinal.right).attr("height", bbox.height + bgPaddingFinal.top + bgPaddingFinal.bottom).attr("x", bbox.x - bgPaddingFinal.left).attr("y", -bgPaddingFinal.top).attr("fill", "white").attr("fill-opacity", 0.7);
      }
    }
  }]);
  return d3NoteText;
